...
using NetAppCommon.Helpers.Lists;
...
public abstract class FileSystemWatcherCommon : IFileSystemWatcherCommon
{
    ...
    /// </param>
    public virtual void Watch(string path,
        NotifyFilters notifyFilters = NotifyFilters.Attributes | NotifyFilters.CreationTime |
        ...
        if (Directory.Exists(path))
        {
            ...
            if (null != filter && !string.IsNullOrWhiteSpace(filter))
            {
                List<string> filterList =
                    ListsHelper.GetInstance().ConvertToListOfString(filter, new[] { ',', ';', '|' });
                if (!(filterList is { Count: > 0 }))
                {
    ...
    public virtual void TimeoutFileAction<T>(Func<T> func, string filePath) =>
        FileHelper.GetInstance().TimeoutAction(func, filePath);

    public T TimeoutFileActionGet<T>(Func<T> func, string filePath) =>
        FileHelper.GetInstance().TimeoutActionReturn(func, filePath);
        ...
        try
        {
            if (null != filePath && !string.IsNullOrWhiteSpace(filePath) && File.Exists(filePath))
            {
                ...
                if (!string.IsNullOrWhiteSpace(fileName))
                {
                    ...
                    fileName = Regex.Replace(Regex.Replace(fileName, @"\.+", "."), @"\[+|\]+", string.Empty)
                        .Replace(Path.GetExtension(filePath), string.Empty);
                    return
    ...
    ///     New filename as string or null
    /// </returns>
    public virtual async Task<string> SetFileNameAsync(string filePath) => await Task.Run(() => SetFileName(filePath));
        ...
        try
        {
            if (null != filePath && !string.IsNullOrWhiteSpace(filePath) && File.Exists(filePath))
            {
                ...
                var destFilePath = Path.Combine(Path.GetDirectoryName(filePath)!,
                    $"{@".."}{Path.DirectorySeparatorChar}", "error", DateTime.Now.Year.ToString(),
                    DateTime.Now.Month.ToString(), DateTime.Now.Day.ToString(), destFileName);
    ...
    public virtual void MoveFileToOutDirectory(string filePath)
    {
        try
        {
            if (null != filePath && !string.IsNullOrWhiteSpace(filePath) && File.Exists(filePath))
            {
                var destFileName = SetFileName(filePath);
                var destFilePath = Path.Combine(Path.GetDirectoryName(filePath)!,
                    $"{@".."}{Path.DirectorySeparatorChar.ToString()}", "out", DateTime.Now.Year.ToString(),
                    DateTime.Now.Month.ToString(), DateTime.Now.Day.ToString(), destFileName);
                if (!Directory.Exists(Path.GetDirectoryName(destFilePath)))
                {
                    Directory.CreateDirectory(Path.GetDirectoryName(destFilePath)!);
        ...
        try
        {
            if (null != filePath && !string.IsNullOrWhiteSpace(filePath) && File.Exists(filePath))
            {
                ...
                var destinationBackupFileName = Path.Combine(Path.GetDirectoryName(filePath)!,
                    $"{@".."}{Path.DirectorySeparatorChar.ToString()}", "error", DateTime.Now.Year.ToString(),
                    DateTime.Now.Month.ToString(), DateTime.Now.Day.ToString(), "backup", SetFileName(filePath));
                var destFilePath = Path.Combine(Path.GetDirectoryName(filePath)!,
                    $"{@".."}{Path.DirectorySeparatorChar.ToString()}", "error", DateTime.Now.Year.ToString(),
                    DateTime.Now.Month.ToString(), DateTime.Now.Day.ToString(), destFileName);
                if (!Directory.Exists(Path.GetDirectoryName(destFilePath)))
